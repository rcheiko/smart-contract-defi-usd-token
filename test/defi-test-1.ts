...
describe("Defi - token/swap Smart Contract", function () {
  ...
  it("Should approve/withdraw 1 lty for 1 usd with test Wallet", async function () {
    ...
    expect(JSON.parse(balanceLTYTestWallet)).to.below(0.06 * this.decimals_token).to.above(0.05 * this.decimals_token)
  ...
  it("Should transfer lty token -> Test wallet to Owner wallet and hold 1 year", async function () {
    ...
    await this.tokenDeployed.connect(this.testWallet).transfer(this.owner.address, BigInt(0.05 * this.decimals_token))

    const balanceLTYOwnerWallet1 = await this.tokenDeployed.balanceOf(this.owner.address)
    expect(JSON.parse(balanceLTYOwnerWallet1)).to.equal(0.05 * this.decimals_token)

    const balanceLTYTestWallet = await this.tokenDeployed.balanceOf(this.testWallet.address)
    expect(JSON.parse(balanceLTYTestWallet)).to.below(0.01 * this.decimals_token)

    await time.increase(60 * 60 * 24 * 365); // 1 year

    const balanceLTYOwnerWallet2 = await this.tokenDeployed.balanceOf(this.owner.address)
    expect(JSON.parse(balanceLTYOwnerWallet2)).to.equal(0.0525 * this.decimals_token) // + 5 % of 0.05 is 0.0525
  })
});
